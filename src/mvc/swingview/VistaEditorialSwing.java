...
public class VistaEditorialSwing
		implements ActionListener, KeyListener, ListSelectionListener, TableModelListener, MouseListener {

	private Controlador controlador;
	private JFrame ventana;
	private JLabel tituloFuncion = new JLabel("MANTENIMIENTO DE EDITORIALES");
	private JLabel etiquetaNuevo = new JLabel("Nueva editorial: ");
	...
	public VistaEditorialSwing(JFrame ventana, Controlador controlador) {
	...
	private void cargarDatosEnTabla(DefaultTableModel modelo) {
		try {
			ResultSet datos = controlador.obtenerDatosMasMetadatosEditorial();
	...
	public void keyPressed(KeyEvent ke) {
		if (KeyEvent.getKeyText(ke.getKeyCode()).equals("Enter")) {
			if (!modoEdicion) {
				...
				modificarRegistro();
				deshabilitarPanelEdicion();
				etiquetaNuevo.setText("Nueva editorial: ");
	...
	public void actionPerformed(ActionEvent ae) {
		...
		case "Editar":
			...
			tabla.setEnabled(false);
			deshabilitarPanelCRUD();
			etiquetaNuevo.setText("Editar editorial: ");
	...
	// habilita/deshabilita el panel de edicion
		...
		private void modificarRegistro() {
			String feedback = controlador.updateEditorial((int) tabla.getValueAt(tabla.getSelectedRow(), 0),
					textoNuevo.getText());
			if (feedback.equals("Edición de editorial correcta.")) {
		...
		private void modificarRegistro(int claveModificacion, String valorModificacion, int fila, int columna) {
			String pregunta = "Ha modificado el nombre de la editorial cuyo codigo es: " + claveModificacion + "\n¿Está seguro?";
			String feedback = "No se ha modificado la editorial.";
			if (preguntarUsuario(pregunta) == JOptionPane.YES_OPTION) {
				feedback = controlador.updateEditorial(claveModificacion, valorModificacion);
			} else {
		...
		private void borrarRegistro() {
			...
			String pregunta = "Va a borrar la editorial con codigo " + tabla.getValueAt(tabla.getSelectedRow(), 0)
					+ "\n¿Está seguro?";
			String feedback = "No se ha borrado la editorial.";
			if (preguntarUsuario(pregunta) == JOptionPane.YES_OPTION) {
				feedback = controlador.deleteEditorial((int) tabla.getValueAt(tabla.getSelectedRow(), 0));
				if (feedback.equals("Borrado de editorial correcto.")) {
		...
		private void registrarNuevo() {
			...
			// al usuario
			informarUsuario(controlador.insertEditorial(textoNuevo.getText()));
