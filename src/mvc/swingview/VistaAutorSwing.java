...
public class VistaAutorSwing
	...
	// Este panel aglutina todos los componentes de la funcion. Es reutilizable en
	// VistaMenuSwing
	private JPanel panelCentral;
	// Este objeto Dimension determina el tamaño necesario del marco (JFrame),
	// teniendo en cuenta panelCentral
	private Dimension tamanoMarcoRequerido;

	public VistaAutorSwing(JFrame ventana, Controlador controlador) {
	...
	private void cargarDatosEnTabla(DefaultTableModel modelo) {
		try {
			// pedir al controlador los datos
			...
			// los ResultSet empiezan desde 1
	...
	private void modificarRegistro() {
		String feedback = controlador.updateAuthor((int) tabla.getValueAt(tabla.getSelectedRow(), 0),
				textoNuevo.getText());
		if (feedback.equals("Edición de autor correcta.")) {
	...
	private void modificarRegistro(int claveModificacion, String valorModificacion, int fila, int columna) {
		String pregunta = "Ha modificado el nombre del autor cuyo codigo es: " + claveModificacion + "\n¿Está seguro?";
		String feedback = "No se ha modificado el autor.";
		if (preguntarUsuario(pregunta) == JOptionPane.YES_OPTION) {
			feedback = controlador.updateAuthor(claveModificacion, valorModificacion);
		} else {
	...
	private void borrarRegistro() {
		...
		String pregunta = "Va a borrar el autor con codigo " + tabla.getValueAt(tabla.getSelectedRow(), 0)
				+ "\n¿Está seguro?";
		String feedback = "No se ha borrado el autor.";
		if (preguntarUsuario(pregunta) == JOptionPane.YES_OPTION) {
			feedback = controlador.deleteAuthor((int) tabla.getValueAt(tabla.getSelectedRow(), 0));
	...
	private void registrarNuevo() {
		...
		// al usuario
		informarUsuario(controlador.insertAuthor(textoNuevo.getText()));
